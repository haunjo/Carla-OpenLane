...
class OpenLaneV2_subset_A_Dataset(Custom3DDataset):
    ...
    def load_annotations(self, ann_file):
        ...
        if isinstance(data_infos, dict):
            if self.filter_map_change and self.split == 'train':
                ...
                # print(list(data_infos.keys()))
                # data_infos = [data_infos[('val','10135','315971202449927209')]]
        return data_infos
    ...
    def evaluate(self, results, logger=None, show=False, out_dir=None, **kwargs):
        ...
        """
        # from projects.toponet.models.dense_heads.toponet_head import total_distance
    ...
    def show(self, results, out_dir, score_thr=0.3, show_num=60, **kwargs):
        """Show the results.
        ...
        for idx, result in enumerate(results):
            ...
            # Prepare GT annotation for visualization
            gt_result = info['annotation']
            gt_result = assign_attribute(gt_result)
            gt_result = assign_topology(gt_result)
            ...
            for cam_name, cam_info in info['sensor'].items():
                ...
                # Draw GT
                image_pv_gt = mmcv.imread(image_path, channel_order='rgb')
                image_pv_gt = draw_annotation_pv(
                    cam_name,
                    image_pv_gt,
                    gt_result,
                    cam_info['intrinsic'],
                ...
                pv_imgs_gt.append(image_pv_gt[..., ::-1])

            # Save prediction front cam
            for cam_idx, image in enumerate(pv_imgs[:1]):
                output_path = os.path.join(out_dir, f'{info["segment_id"]}/{info["timestamp"]}/{self.CAMS[cam_idx]}_pred.jpg')
                mmcv.imwrite(image, output_path)

            # Save GT front cam
            for cam_idx, image in enumerate(pv_imgs_gt[:1]):
                output_path = os.path.join(out_dir, f'{info["segment_id"]}/{info["timestamp"]}/{self.CAMS[cam_idx]}_gt.jpg')
                mmcv.imwrite(image, output_path)
            ...
            # Save separate GT and pred BEV images
            bev_img_gt = show_bev_results(gt_lanes, pred_lanes, only='gt', map_size=[-52, 52, -27, 27], scale=20)
            output_path = os.path.join(out_dir, f'{info["segment_id"]}/{info["timestamp"]}/bev_gt.jpg')
            mmcv.imwrite(bev_img_gt, output_path)

            bev_img_pred = show_bev_results(gt_lanes, pred_lanes, only='pred', map_size=[-52, 52, -27, 27], scale=20)
            output_path = os.path.join(out_dir, f'{info["segment_id"]}/{info["timestamp"]}/bev_pred.jpg')
            mmcv.imwrite(bev_img_pred, output_path)

            # Save combined BEV image
            divider = np.ones((bev_img_gt.shape[0], 7, 3), dtype=np.uint8) * 128
            bev_img_combined = np.concatenate([bev_img_gt, divider, bev_img_pred], axis=1)
            output_path = os.path.join(out_dir, f'{info["segment_id"]}/{info["timestamp"]}/bev.jpg')
            mmcv.imwrite(bev_img_combined, output_path)

            # Save topology connection images
