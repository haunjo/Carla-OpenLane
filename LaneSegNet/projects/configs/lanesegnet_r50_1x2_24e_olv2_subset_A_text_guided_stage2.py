"""
Stage 2: Text-Guided Topology Reasoning on OpenLane-V2 (Transfer Learning)

This config fine-tunes the Stage 1 checkpoint on OpenLane-V2 for 24 epochs with FROZEN text components.
Simplified version focusing only on lane segment detection and lane-to-lane topology.

Key features:
- LaneSegNet (not MapElementBucket) - lane segment + topology only
- TextEnhancedRelationshipHead for lane-to-lane topology (lclc_head)
- FROZEN text components (freeze_text_proj=True)
- NO text supervision (loss_text weight = 0.0)
- Loss: L_total = L_lane + L_lclc
- 24 epochs on OpenLane-V2 (real-world data, no GT text labels)
- Loads Stage 1 checkpoint trained on Carla-OLV2

Training strategy:
Stage 1 (Carla-OLV2, 8 epochs): Learn text-aligned topology representations
Stage 2 (OpenLane-V2, 24 epochs): Transfer to real-world with frozen text alignment
"""
...
data_root = 'data/OpenLane-V2/'  # Changed from Carla-OLV2
...
model = dict(
    ...
    lclc_head=dict(
        ...
        loss_rel=dict(
            ...
            alpha=0.25,
            loss_weight=5),
        loss_text=None),  # Stage 2: NO text supervision (set to None)
...
train_pipeline = [
    ...
    dict(type='LoadAnnotations3DLaneSegment',
         ...
         with_lane_text=False,  # Stage 2: No text labels needed
    ...
    dict(type='CustomCollect3D', keys=[
        ...
        'gt_instance_masks', 'gt_lane_left_type', 'gt_lane_right_type'])  # Removed gt_lane_text_labels
]
...
evaluation = dict(interval=4, start=8, pipeline=test_pipeline)  # Eval at epochs 8, 12, 16, 20, 24
...
log_level = 'INFO'
work_dir = None
# Load Stage 1 checkpoint (8 epochs on Carla-OLV2)
load_from = 'work_dirs/lanesegnet_8e_carla_laneseg_text_guided_1031/epoch_8.pth'
resume_from = None
workflow = [('train', 1)]
