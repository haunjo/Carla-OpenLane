...
"""
Ablation Study: Self-Attention + Cross-Attention

This config adds lane self-attention before text cross-attention.

Changes from baseline:
- use_self_attn=True (adds lane-to-lane context aggregation)
- n_heads=1 (keep single-head cross-attention for fair comparison)
- Self-attention enables contextual topology reasoning

Expected improvements:
- Lane context complements text semantics
- Better topology prediction via extrinsic spatial relationships
- Especially helpful for intersection scenes
"""
...
model = dict(
    ...
    lsls_head=dict(
        ...
        use_self_attn=True,  # Lane self-attention before cross-attention
        n_heads=1,  # Single-head cross-attention (for fair comparison)
        loss_rel=dict(
            type='FocalLoss',
            use_sigmoid=True,
            gamma=2.0,
