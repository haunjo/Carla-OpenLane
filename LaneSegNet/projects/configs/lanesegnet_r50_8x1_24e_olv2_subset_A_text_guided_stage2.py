...
Fine-tuning on OpenLane-V2 after CARLA pre-training with text-guided topology reasoning.

Load from: work_dirs/lanesegnet_text_guided_lste_1stage/latest.pth (CARLA checkpoint)

Key changes from Stage 1:
- Text guidance DISABLED (vision-only model)
- freeze_text_proj=True (freeze text components)
- No loss_text (text supervision removed)
- No gt_lane_text_labels in data pipeline
"""
...
model = dict(
    ...
    lsls_head=dict(
        ...
        freeze_text_proj=True,  # Freeze text components in Stage 2
        loss_rel=dict(
            type='FocalLoss',
            use_sigmoid=True,
            gamma=2.0,
            alpha=0.25,
            loss_weight=5)),
        # loss_text removed - no text supervision in Stage 2
    bbox_head=dict(
...
train_pipeline = [
    ...
    dict(type='CustomCollect3D', keys=[
        ...
        # gt_lane_text_labels removed - no text supervision in Stage 2
]
...
log_level = 'INFO'
work_dir = None
# Load CARLA pre-trained checkpoint
load_from = 'work_dirs/lanesegnet_text_guided_lste_1stage/latest.pth'
resume_from = None
workflow = [('train', 1)]
