...
"""
Ablation Study: Multi-head Cross-Attention (H=4) - Stage 2 Fine-tuning

This config fine-tunes the Stage 1 model on OpenLane-V2 with frozen text components.

Changes from Stage 1:
- Dataset: OpenLane-V2 Subset A (real-world data)
- freeze_text_proj=True (freeze SigLIP, text MLP, temperature)
- No text supervision loss (gt_lane_text_labels assigned -1)
- 24 epochs fine-tuning
- Load from Stage 1 checkpoint

Key insight:
- Visual components adapt to real-world appearance
- Text components remain frozen to preserve semantic anchors
- This prevents catastrophic forgetting of CARLA topology knowledge
"""
...
class_names = ['lane_segment', 'ped_crossing', 'road_boundary']
...
model = dict(
    type='LaneSegNetMapElementBucket',
    img_backbone=dict(
        type='ResNet',
        depth=50,
        num_stages=4,
    ...
    lsls_head=dict(
        ...
        shared_param=False,  # lane-to-lane does NOT share parameters with lane-to-traffic
        text_embed_path='text_embeddings_siglip.pt',
        text_embed_dim=1152,  # SigLIP-so400m
        proj_dim=256,
        temperature_init=0.07,
        freeze_text_proj=True,  # Stage 2: FREEZE text components (semantic anchors)
        use_self_attn=False,  # No self-attention (only multi-head cross-attn)
        n_heads=4,  # Multi-head attention (H=4)
        loss_rel=dict(
            type='FocalLoss',
            use_sigmoid=True,
            gamma=2.0,
            alpha=0.25,
            loss_weight=5),
        loss_text=None),  # No text loss in Stage 2 (no GT text labels)
    bbox_head=dict(
    ...
    area_head=dict(
        ...
        transformer=dict(
            type='MapTRTransformerDecoderOnly',
            embed_dims=_dim_,
            pts_dim=pts_dim,
            decoder=dict(
                type='MapTRDecoder',
...
train_pipeline = [
    ...
    dict(type='LoadAnnotations3DLaneSegment',
         ...
         with_lane_text=True,  # Still load, but will be assigned -1 in dataset
         with_bbox=True, with_label=True, with_lane_lste_adj=True, with_area=True),
    # dict(type='PhotoMetricDistortionMultiViewImage'),  # Same as Stage 1
    dict(type='CropFrontViewImageForAv2'),
    dict(type='RandomScaleImageMultiViewImage', scales=[0.33]),  # Same as Stage 1
    ...
    dict(type='CustomCollect3D', keys=[
        ...
        'gt_lane_text_labels',  # -1 for all lanes (no text labels in OpenLane-V2)
...
lr_config = dict(
    ...
    warmup_ratio=1.0 / 3,
    min_lr_ratio=1e-3)
total_epochs = 24  # Stage 2: 24 epochs on OpenLane-V2
evaluation = dict(interval=24, pipeline=test_pipeline)  # Same as baseline: evaluate at epoch 24
...
checkpoint_config = dict(interval=1, max_keep_ckpts=8)  # Same as baseline

dist_params = dict(backend='nccl')
log_level = 'INFO'
work_dir = 'work_dirs/ablation_multihead_h4_stage2'

# IMPORTANT: Load Stage 1 checkpoint
# Option 1: Use matched Stage 1 checkpoint (ablation_multihead_h4_stage1)
# load_from = 'work_dirs/ablation_multihead_h4_stage1/latest.pth'

# Option 2: Start from scratch (train Stage 1 first)
load_from = None  # Train Stage 1 first with ablation_multihead_h4.py

# Option 3: Use existing single-head checkpoint (may have parameter mismatch)
# load_from = 'work_dirs/lanesegnet_1stage_text_guided_mapele_bucket/latest.pth'

resume_from = None
workflow = [('train', 1)]

# NOTE: `auto_scale_lr` is for automatically scaling LR,
# base_batch_size = (1 GPU) x (2 samples per GPU)
