...
"""
CARLA Stage 1 - Text-guided lane-to-traffic element topology reasoning
Text guidance applied ONLY to lste_head (lane-TE topology), not lsls_head.

Multi-head approach:
- Deformable DETR for traffic element detection (13 classes)
- LaneSegNet head for lane segment detection
- RelationshipHead for lane-lane topology reasoning (naive)
- TextEnhancedRelationshipHead for lane-traffic element topology reasoning (text-guided)
- MapTR head for area detection
"""
...
model = dict(
    ...
    lste_head=dict(
        ...
        in_channels_o2=_dim_,
        shared_param=False,
        # Text-guided topology reasoning
        text_embed_path='text_embeddings_siglip_optimized.pt',
        freeze_text_proj=False,  # Train text components in Stage 1
        temperature_init=0.07,  # CLIP/SigLIP standard temperature
        loss_rel=dict(
            type='FocalLoss',
            use_sigmoid=True,
            gamma=2.0,
        ...
        loss_text=dict(
            type='CrossEntropyLoss',
            loss_weight=0.05)),  # Reduced from 0.1 to 0.05 for stability
    area_head=dict(
        type='MapTRHead',
        bev_h=bev_h_,
        bev_w=bev_w_,
...
train_pipeline = [
    ...
    dict(type='LoadAnnotations3DLaneSegment',
         ...
         with_bbox=True, with_label=True, with_lane_lste_adj=True, with_area=True, with_lane_text=True),
    ...
    dict(type='CustomCollect3D', keys=[
        ...
        'gt_lane_text_labels'])  # Add text labels for text-guided topology
]
...
lr_config = dict(
    ...
    min_lr_ratio=1e-3)
total_epochs = 12
evaluation = dict(interval=4, start=8, pipeline=test_pipeline)
