"""
Stage 1: Text-Guided Topology Reasoning on Carla-OLV2 (Lane Segment Task Only)
...
- Loss: L_total = L_lane + L_lclc + 0.1*L_text
- 8 epochs on Carla-OLV2 with explicit text labels

After training, use checkpoint for Stage 2 (OpenLane-V2 fine-tuning with frozen text).
"""
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file=data_root + 'data_dict_carla_argoverse2_train_lanesegnet.pkl',
    ...
    val=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file=data_root + 'data_dict_carla_argoverse2_val_lanesegnet.pkl',
...
evaluation = dict(interval=4, start=4, pipeline=test_pipeline)  # Eval at epoch 4, 8
