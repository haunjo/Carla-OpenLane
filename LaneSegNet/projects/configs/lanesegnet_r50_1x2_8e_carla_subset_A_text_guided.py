...
"""
Stage 1: Text-Guided Topology Reasoning on Carla-OLV2

This config trains LaneSegNet with TextEnhancedRelationshipHead for 8 epochs on Carla-OLV2.

Key features:
- TextEnhancedRelationshipHead for lane-to-lane topology (lsls_head)
- Uses pre-generated SigLIP text embeddings (text_embeddings_siglip.pt)
- Text supervision via L_text = CrossEntropy(attention, GT_template_ID)
- Loss: L_total = L_lane + L_lsls + 0.1*L_text + 0.1*(L_bbox + L_lste)
- Traffic element loss weights reduced 10x (mitigate 3-class Carla vs 13-class OLV2 bias)
- 8 epochs on Carla-OLV2 with explicit text labels

Multi-head approach:
- Deformable DETR for traffic element detection
- LaneSegNet head for lane segment detection
- TextEnhancedRelationshipHead for lane-to-lane topology (NEW)
- RelationshipHead for lane-to-traffic-element topology
- MapTR head for area (ped crossing and road boundary) detection

After training, use the checkpoint for Stage 2 (OpenLane-V2 fine-tuning).
"""
...
model = dict(
    ...
    lsls_head=dict(
        ...
        shared_param=True,  # lane-to-lane uses shared parameters
        text_embed_path='text_embeddings_siglip.pt',
        text_embed_dim=1152,  # SigLIP-so400m
        proj_dim=256,
        temperature_init=0.07,
        freeze_text_proj=False,  # Stage 1: train text components
        loss_rel=dict(
            type='FocalLoss',
            use_sigmoid=True,
            gamma=2.0,
    ...
    bbox_head=dict(
        ...
        loss_cls=dict(
            ...
            gamma=2.0,
            alpha=0.25,
            loss_weight=0.1),  # Reduced from 1.0 (3-class bias mitigation)
        loss_bbox=dict(type='L1Loss', loss_weight=0.25),  # Reduced from 2.5
        loss_iou=dict(type='GIoULoss', loss_weight=0.1),  # Reduced from 1.0
    ...
    lste_head=dict(
        ...
        loss_rel=dict(
            ...
            gamma=2.0,
            alpha=0.25,
            loss_weight=0.5)),  # Reduced from 5.0 (3-class traffic element bias)
    area_head=dict(
        type='MapTRHead',
        bev_h=bev_h_,
        bev_w=bev_w_,
...
train_pipeline = [
    ...
    dict(type='LoadAnnotations3DLaneSegment',
         ...
         with_lane_text=True,  # Load GT text template labels for Stage 1
         with_bbox=True, with_label=True, with_lane_lste_adj=True, with_area=True),
    # dict(type='PhotoMetricDistortionMultiViewImage'),
    dict(type='CropFrontViewImageForAv2'),
    dict(type='RandomScaleImageMultiViewImage', scales=[0.33]),
    ...
    dict(type='CustomCollect3D', keys=[
        ...
        'gt_lane_text_labels',  # Add text labels for Stage 1
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file=data_root + 'data_dict_carla_train_argoverse2_ls.pkl',
    ...
    val=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file=data_root + 'data_dict_carla_val_argoverse2_ls.pkl',
...
lr_config = dict(
    ...
    warmup_ratio=1.0 / 3,
    min_lr_ratio=1e-3)
total_epochs = 8  # Stage 1: 8 epochs on Carla-OLV2
evaluation = dict(interval=4, start=4, pipeline=test_pipeline)  # Evaluate at epoch 4, 8
...
checkpoint_config = dict(interval=2, max_keep_ckpts=5, save_last=True)
