...
"""
OpenLane-V2 Stage 2 - Fine-tuning after text-guided pre-training
Based on: lanesegnet_r50_1x2_8e_carla_subset_A_mapele_bucket_lste_text.py

Changes from Stage 1:
1. Data: Carla-OpenLane → OpenLane-V2
2. lste_head: freeze_text_proj=False → True
3. lste_head: Remove loss_text
4. Pipeline: Remove with_lane_text=True and gt_lane_text_labels
5. Training: 8 epochs → 24 epochs
6. Load from: work_dirs/lanesegnet_text_guided_lste_1stage/latest.pth
"""
...
data_root = 'data/OpenLane-V2/'  # Changed from Carla-OpenLane
...
model = dict(
    ...
    bbox_head=dict(
        ...
        num_query=100,
        num_classes=13,  # Match OpenLane-V2 (13 traffic element classes)
        in_channels=_dim_,
    ...
    lste_head=dict(
        ...
        in_channels_o2=_dim_,
        shared_param=False,
        # Text-guided topology reasoning - FROZEN for Stage 2
        text_embed_path='text_embeddings_siglip_optimized.pt',
        freeze_text_proj=True,  # CHANGED: Freeze text components in Stage 2
        loss_rel=dict(
            type='FocalLoss',
            use_sigmoid=True,
            gamma=2.0,
            alpha=0.25,
            loss_weight=5)),
        # REMOVED: loss_text (no text supervision in Stage 2)
    area_head=dict(
        type='MapTRHead',
        bev_h=bev_h_,
        bev_w=bev_w_,
...
train_pipeline = [
    ...
    dict(type='LoadAnnotations3DLaneSegment',
         ...
         # REMOVED: with_lane_text=True (no text labels in OpenLane-V2)
    ...
    dict(type='CustomCollect3D', keys=[
        ...
        # REMOVED: 'gt_lane_text_labels' (no text labels in OpenLane-V2)
]
...
data = dict(
    samples_per_gpu=2,
    workers_per_gpu=8,  # Changed from 4 to match OpenLane-V2 baseline
    train=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file=data_root + 'data_dict_subset_A_train_ls.pkl',  # Changed from Carla
    ...
    val=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file=data_root + 'data_dict_subset_A_val_ls.pkl',  # Changed from Carla
...
optimizer = dict(
    type='AdamW',
    lr=2e-4,  # Changed from 1e-4 to match OpenLane-V2 baseline
    paramwise_cfg=dict(
        custom_keys={
...
lr_config = dict(
    ...
    warmup_ratio=1.0 / 3,
    min_lr_ratio=1e-3)
total_epochs = 24  # Changed from 8
evaluation = dict(interval=24, pipeline=test_pipeline)  # Changed from interval=4, start=8
...
checkpoint_config = dict(interval=1, max_keep_ckpts=8)  # Changed from interval=2, max_keep_ckpts=5

dist_params = dict(backend='nccl')
log_level = 'INFO'
work_dir = None
load_from = 'work_dirs/lanesegnet_text_guided_lste_1stage/latest.pth'  # ADDED: Load from Stage 1
resume_from = None
workflow = [('train', 1)]
