...
class LaneSegNetMapElementBucket(MVXTwoStageDetector):
    ...
    def forward_train(self,
        ...
        if self.lsls_head is not None:
            # Pass gt_text_labels only if available (for text-guided topology reasoning)
            lsls_kwargs = {}
            if gt_lane_text_labels is not None and hasattr(self.lsls_head, 'text_proj'):
                lsls_kwargs['gt_text_labels'] = gt_lane_text_labels

            lsls_losses = self.lsls_head.forward_train(
                lane_feats, lane_assign_result, lane_feats, lane_assign_result,
                gt_lane_adj, **lsls_kwargs)
