...
class OpenLaneV2_subset_A_LaneSegNet_Dataset(Custom3DDataset):
    ...
    def get_ann_info(self, index):
        ...
        # Load BEFORE processing areas (ped_crossing) to ensure correct indexing
        if 'lane_text_labels' in ann_info:
            gt_lane_text_labels = list(ann_info['lane_text_labels'])
        else:
            gt_lane_text_labels = []
        ...
        for idx, lane in enumerate(ann_info['lane_segment']):
            ...
            # Add -1 if no text labels available
            if len(gt_lane_text_labels) == 0:
        ...
        for area in ann_info['area']:
            if area['category'] == 1 and 'ped_crossing' in self.LANE_CLASSES:
                ...
                # Ped crossing has no text label
                gt_lane_text_labels.append(-1)
