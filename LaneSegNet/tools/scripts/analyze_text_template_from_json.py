"""
Analyze text template distribution from JSON files in Carla-OLV2 dataset.

Directly reads from *-ls.json files instead of pickle.
"""
...
def analyze_json_distribution(data_dir, split_name="train"):
    """Analyze template_id distribution from JSON files."""

    print(f"\n{'='*80}")
    print(f"Analyzing Carla-OLV2 {split_name.upper()} Dataset (from JSON)")
    print(f"Directory: {data_dir}")
    print(f"{'='*80}\n")

    # Find all *-ls.json files
    json_pattern = os.path.join(data_dir, "**/info/*-ls.json")
    json_files = glob.glob(json_pattern, recursive=True)

    print(f"Found {len(json_files)} JSON files")

    if len(json_files) == 0:
        print(f"❌ No *-ls.json files found in {data_dir}")
        return
    ...
    for json_file in json_files:
        ...
        try:
            ...
            if 'annotation' not in data or 'lane_segment' not in data['annotation']:
                continue

            lane_segments = data['annotation']['lane_segment']
    ...
    if errors:
        ...
        for json_file, error in errors[:5]:
    ...
    if len(template_ids) == 0:
        print("\n❌ No template_id found in lane segments!")
        print("   Make sure carla2openlanev2.py was run with the updated version.")
        return

    print(f"\n✓ Found template_id in {len(template_ids)} lane segments")
    print(f"  Total JSON files: {total_frames}")
    print(f"  Files with lanes: {frames_with_lanes}")
    print(f"  Files with template_id: {frames_with_template}")
    print(f"  Average lanes per file: {len(template_ids) / frames_with_template:.2f}")
    ...
    for template_id in range(9):
        ...
        print(f"  Count: {count:6d} ({percentage:6.2f}%) {bar}")
    ...
    for curv_id in range(3):
        ...
        percentage = (count / total) * 100
        bar = '█' * int(percentage / 2)
        print(f"{CURVATURE_NAMES[curv_id]:12s}: {count:6d} ({percentage:6.2f}%) {bar}")
    ...
    for inter_id in range(3):
        ...
        percentage = (count / total) * 100
        bar = '█' * int(percentage / 2)
        print(f"{INTERSECTION_NAMES[inter_id]:12s}: {count:6d} ({percentage:6.2f}%) {bar}")
    ...
    print(f"Min count: {min_count:6d} (Template {template_counts.index(min_count)})")
    print(f"Max count: {max_count:6d} (Template {template_counts.index(max_count)})")
    ...
    if rare_classes:
        ...
        for cls in rare_classes:
            ...
            print(f"  - Template {cls} ({TEXT_TEMPLATES[cls]}): {count} ({count/total*100:.2f}%)")
    ...
    ax1 = axes[0]
    template_counts = [counter.get(i, 0) for i in range(9)]
    bars1 = ax1.bar(range(9), template_counts, color='steelblue', edgecolor='black', linewidth=0.5)
    ax1.set_xlabel('Template ID', fontsize=12, fontweight='bold')
    ax1.set_ylabel('Count', fontsize=12, fontweight='bold')
    ax1.set_title(f'Template Distribution - {split_name.upper()} ({total:,} lanes)',
    ...
    bars2 = ax2.bar(range(3), curv_counts, color=colors, edgecolor='black', linewidth=0.5)
    ax2.set_xlabel('Curvature Type', fontsize=12, fontweight='bold')
    ax2.set_ylabel('Count', fontsize=12, fontweight='bold')
    ax2.set_title(f'Curvature Distribution - {split_name.upper()}',
                  fontsize=14, fontweight='bold')
    ax2.set_xticks(range(3))
    ax2.set_xticklabels(['Straight', 'Left-turn', 'Right-turn'], fontsize=11)
    ...
    colors = ['#95a5a6', '#f39c12', '#e67e22']  # gray, yellow, orange
    bars3 = ax3.bar(range(3), inter_counts, color=colors, edgecolor='black', linewidth=0.5)
    ax3.set_xlabel('Intersection Context', fontsize=12, fontweight='bold')
    ax3.set_ylabel('Count', fontsize=12, fontweight='bold')
    ax3.set_title(f'Intersection Context - {split_name.upper()}',
                  fontsize=14, fontweight='bold')
    ax3.set_xticks(range(3))
    ax3.set_xticklabels(['Regular', 'Connector', 'Intersection'], fontsize=11)
    ...
    return {
        ...
        'files_with_lanes': frames_with_lanes,
        'files_with_template': frames_with_template,
...
if __name__ == "__main__":
    ...
    # Default directories
    train_dir = "data/Carla-OLV2/train"
    val_dir = "data/Carla-OLV2/val"
    ...
    # Analyze train set
    try:
        stats = analyze_json_distribution(data_dir, split_name)

        # Also analyze val set if train was analyzed
        if split_name == "train" and os.path.exists(val_dir):
            print("\n" + "="*80)
            print("Also analyzing validation set...")
            print("="*80)
            val_stats = analyze_json_distribution(val_dir, "val")
