            ...
            for l1_idx, lclc in enumerate(gt_lclc):
                for l2_idx, connected in enumerate(lclc):
                    if connected:
                        ...
                        image = cv2.arrowedLine(image, (p1[1], p1[0]), (p2[1], p2[0]), GT_ARROW, max(round(scale * 0.1), 1), tipLength=0.1)
    
    if only is None or only == 'pred':
        for lane in pred_lane:
            draw_coor = (scale * (-lane[:, :2] + np.array([map_size[1], map_size[3]]))).astype(int)
            image = cv2.polylines(image, [draw_coor[:, [1,0]]], False, (255, 0, 0), max(round(scale * 0.2), 1))
            image = cv2.circle(image, (draw_coor[0, 1], draw_coor[0, 0]), max(2, round(scale * 0.5)), (255, 0, 0), -1)
